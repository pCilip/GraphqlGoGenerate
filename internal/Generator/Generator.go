...
func (generator *Generator) addImportPath(key, path string) string {
	newPath := fmt.Sprintf("%s/%s", generator.ProjectName, path2.Dir(path))
	generator.Imports[key] = newPath
	return newPath
}
...
func (generator *Generator) Render(object Schema2.FullType, dir string) bool {
	...
	parentPackage := generator.addImportPath(*object.Name, filePath)
	...
	for _, value := range object.InputFields {
		...
		endType := BuildParamType(generator, parentPackage, value, field)
...
func (generator *Generator) RenderObject(object Schema2.FullType, dir string) bool {
	...
	for _, value := range object.Fields {
		...
		for _, param := range value.Args {
			...
			endType := BuildParamType(generator, parentPackage, param, genParam)
		...
		endType := BuildFieldType(generator, parentPackage, value, returnTypes)
